    ...
    public function export($mouvements) {
        ...
        foreach($db2Mouvements as $identifiantPeriode => $volumes) {
            ...
            foreach($this->structure as $file => $infos) {
                $ligne = substr($periode, 0, 4).",".substr($periode, 4, 2).",".str_replace("C", "", $identifiant).",,0";
        ...
        foreach($db2MouvementsExport as $identifiantPeriode => $infos) {
            ...
            foreach($infos as $pays => $produits) {
                $ligne = substr($periode, 0, 4).",".substr($periode, 4, 2).",".str_replace("C", "", $identifiant).",,0,".$compteur.",".$pays*1;
        ...
        foreach($db2CRD as $identifiantPeriode => $centilisations) {
            ...
            foreach($centilisations as $centilisation => $sorties) {
                $ligne = substr($periode, 0, 4).",".substr($periode, 4, 2).",".str_replace("C", "", $identifiant).",,0,".$compteur.",\"".$centilisation."\"";
        ...
        foreach($db2Total as $identifiantPeriode => $total) {
            ...
            $ligne = substr($periode, 0, 4).",".substr($periode, 4, 2).",".str_replace("C", "", $identifiant).",,0,\"\",0,0,0,".$total["prix_ht"].",".$total["tva"].",".$total["prix_ttc"].",".$total["quantite"].",".$total["prix_ttc"];
            foreach($this->structure as $file => $infos) {
                foreach($this->produits as $produit) {
                    $volume = 0;
