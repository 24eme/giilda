            ...
            <?php foreach ($factures->getRawValue() as $facture) :
                ...
                <tr>
                    <td><?php $fc = FactureClient::getInstance();
                        $d = format_date($facture->value[FactureEtablissementView::VALUE_DATE_FACTURATION],'dd/MM/yyyy').' (créée le '.$fc->getDateCreation($facture->id).')';
                        echo link_to($d, array('sf_route' => 'facture_pdf', 'identifiant' => $facture->key[FactureEtablissementView::KEYS_FACTURE_ID])); ?>
                    </td>
