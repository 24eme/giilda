    ...
    <div data-hash="<?php echo $detail->getHash() ?>"  class="panel panel-default col_recolte<?php if ($active): ?> active col_active<?php endif; ?> <?php echo ($detail->isEdited()) ? 'col_edited panel-success' : '' ?>" data-input-focus="<?php echo $tabindex; ?>" data-cssclass-rectif="<?php echo ($form->getObject()->getDocument()->isRectificative()) ? VersionnerCssClass() : '' ?>" style="margin-right: 10px;">
                    ...
                    <?php foreach ($form['stocks_debut'] as $key => $subform): ?>
                        ...
                        if ($key != 'instance'):
                            ...
                            <h4 class="form-group form-group-xs <?php // echo isVersionnerCssClass($subform->getObject(), $key)             ?>">
                                <?php echo $form['stocks_debut'][$key]->render(array('data-val-defaut' => $form['stocks_debut'][$key]->getValue(), 'class' => $form['stocks_debut'][$key]->getWidget()->getAttribute('class') . $class . ' somme_detail ', 'tabindex' => $tabindex)) ?>
                            </h4> 
                            <?php
                            $tabindex++;
                        endif;
                    ...
                    <div class="groupe p_gris" data-groupe-id="3">
                        ...
                        <ul class="list-unstyled">
                            ...
                            foreach ($form['entrees'] as $key => $subform):
                                ...
                                ?>
                                <li class="form-group form-group-xs <?php echo isVersionnerCssClass($form->getObject()->entrees, $key) ?>">
                                <?php echo $subform->render(array('data-val-defaut' => $subform->getValue(), 'class' => $subform->getWidget()->getAttribute('class') . $class . ' somme_detail bold_on_blur', 'tabindex' => $tabindex, 'data-previousfocus' => $datapreviousfocus)); ?>
                                </li>
                                <?php
                                $tabindex++;
                            endforeach;
        ...
        <?php elseif ($key == "export"): ?>
                                    ...
                                    <?php elseif ($key == "cooperative"): ?>
                                        <input type="text" class="btn_detail pointer input-float somme_detail bold_on_blur drm_input_details <?php echo $class; ?>" data-title="Details des cooperatives" data-href="<?php echo url_for("drm_cooperative_details", $form->getObject()) ?>" readonly="readonly" value="<?php echoFloat($detail->sorties->cooperative); ?>"/>
                                    <?php else: ?>
                                        <?php echo $subform->render(array('data-val-defaut' => $subform->getValue(), 'class' => $subform->getWidget()->getAttribute('class') . $class . ' somme_detail bold_on_blur', 'tabindex' => $tabindex)); ?>
...
<?php endforeach; ?>
                ...
                <div class="col_btn list-group-item list-group-item-xs invisible">
                    ...
                    <button class="btn_reinitialiser btn_annuler btn-block btn_majeur btn btn-xs btn-danger" tabindex="<?php echo $tabindex++; ?>" type="submit">Annuler</button>
                </div>
            </form>
        </div>
    </div>
