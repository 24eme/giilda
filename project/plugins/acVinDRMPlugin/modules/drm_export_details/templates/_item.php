...
<tr id="<?php echo $ligneId ?>">
    ...
    <td class="col-xs-3 typedoc_unshow"   <?php echo (!$docShow) ? '' : 'style="display: none;"' ?>  ></td>
        <td class="form-group <?php if($form['type_document']->hasError()): ?>has-error<?php endif; ?> col-xs-2">    
            <?php
            echo $form['type_document']->renderError();
            echo $form['type_document']->render();
            ?>
        </td>   
        <td class="form-group <?php if($form['numero_document']->hasError()): ?>has-error<?php endif; ?> col-xs-2">    
            <?php
