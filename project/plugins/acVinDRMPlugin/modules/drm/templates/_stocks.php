                ...
                <tbody>
                    ...
                    <?php $i++; ?>
                            <tr class="hamzastyle-item" data-words='<?php echo json_encode(array_merge(array(strtolower($produit->produit_libelle), strtolower($produit->mois))), JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) ?>' id="<?php echo produit_get_id($produit) ?>">
                                ...
                                <td><strong><?php echoFloat($produit->stocks_debut_initial) ?></strong><?php if($produit->exist("stocks_debut_dont_revendique")): ?> (<?php echoFloat($produit->stocks_debut_dont_revendique) ?>) <?php endif; ?></td>
                                ...
                                <td><strong><?php echoFloat($produit->stocks_fin_final) ?></strong><?php if($produit->exist("stocks_fin_dont_revendique")): ?> (<?php echoFloat($produit->stocks_fin_dont_revendique) ?>)<?php endif; ?></td>
                            </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
