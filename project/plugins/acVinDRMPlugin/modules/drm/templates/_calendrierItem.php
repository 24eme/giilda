        ...
        <?php else: ?>
            <div class="<?php echo getEtatDRMPictoCalendrier($isTeledeclarationMode, $calendrier, $periode, $etablissement); ?>">
               ...
               <?php if (hasALink($isTeledeclarationMode, $calendrier, $periode)) : ?> 
                                <a id="<?php echo ($lastDrmToCompleteAndToStart->getRawValue()->periode == $periode)? 'to_autofocus' : ''; ?>" href="<?php echo getEtatDRMHrefCalendrier($isTeledeclarationMode, $calendrier, $periode); ?>" class="btn btn-default btn-block <?php if (hasPopup($isTeledeclarationMode, $calendrier, $periode, $etablissement)): echo 'drm_nouvelle_teledeclaration';
