        ...
        foreach ($configurationDetail->getEntreesSorted() as $key => $value) {
            ...
            if ($value->readable) {
                if (!$value->writable || $disabled || (preg_match($declassementIgp, $certif) && ($key == 'declassement'))
                    || (($certif == 'AUTRES') && !preg_match("#/(TRANQ|EFF|MOU)/#", $this->getObject()->getHash()) && strpos($this->getObject()->getHash(), 'CIDRE') === false && ($key != 'recolte') && ($key != 'revendication') && ($key != 'transfertsrecolte') && ($key != 'regularisation'))
                    || (($certif == 'AUTRE' || $genre == 'VCI') && !preg_match("#/(TRANQ|EFF|MOU)/#", $this->getObject()->getHash()) && $key == 'retourmarchandisetaxees')
