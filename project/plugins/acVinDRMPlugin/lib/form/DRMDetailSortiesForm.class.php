...
class DRMDetailSortiesForm  extends acCouchdbObjectForm {
    ...
    public function configure() {
    	...
    	foreach ($configurationDetail->getSortiesSorted() as $key => $value) {
            ...
            if (strpos($certif, 'AUTRE') === 0 && !preg_match("#/(TRANQ|EFF)/#", $this->getObject()->getHash()) && $this->getObject()->getParent()->code_douane != "BOISSONS_FERMENTEES_AUTRES" && $key != 'distillationusageindustriel' && $key != 'destructionperte' && $key != 'manquant' && $key != 'vracsanscontratsuspendu') {
                $disabled = true;
            }
