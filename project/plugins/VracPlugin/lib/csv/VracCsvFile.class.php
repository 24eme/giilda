...
class VracCsvFile extends CsvFile 
{
  ...
  public function importVracs() {
    ...
    try {
      foreach ($csvs as $line) {
        ...
        $v->courtier_identifiant = 'ETABLISSEMENT-'.$line[self::CSV_CODE_COURTIER];
        $v->type_transaction = $type_transaction;
        
        if (in_array($v->type_transaction, array(self::CSV_TYPE_PRODUIT_VIN_BOUTEILLE)) {
          $v->bouteille_contenance = $line[self::CSV_COEF_CONVERSION_PRIX] * 100;
          $v->bouteilles_quantite = $line[self::CSV_VOLUME_PROPOSE_HL] * $v->bouteille_contenance / 10000;
        } elseif(in_array($v->type_transaction, array(self::CSV_TYPE_PRODUIT_MOUTS,
                                                      self::CSV_TYPE_PRODUIT_VIN_VRAC) {
          $v->jus_quantite = $line[self::CSV_VOLUME_PROPOSE_HL];
        } elseif(in_array($v->type_transaction, array(self::CSV_TYPE_PRODUIT_RAISINS) {
          $v->raisin_quantite = $line[self::CSV_VOLUME_PROPOSE_HL] * $line[self::CSV_COEF_CONVERSION_PRIX];
        }

        $v->prix_unitaire = $line[self::CSV_PRIX_AU_LITRE];

        $v->update(); 

      	$vracs[$v->identifiant] = $v;
      }
    } catch(Execption $e) {
  ...
  public function convertTypeTransaction($type) {
  ...
  public function convertJusQuantite($volume, $unite) {
    if ($unite == self::CSV_UNITE_VOLUME_HL) {
