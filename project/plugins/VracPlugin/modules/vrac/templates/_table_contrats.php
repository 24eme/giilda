...
foreach ($vracs->rows as $value) {    $cpt *= -1;
        ...
        <tr<?php if($cpt > 0) echo ' class="odd"'; ?>>
	      ...
	      <td><?php $vracid = preg_replace('/VRAC-/', '', $elt[1]); echo link_to($vracid, '@vrac_termine?numero_contrat='.$vracid); ?></td>
	       <td><?php echo ($elt[2]) ? link_to($elt[3], 'vrac/rechercheSoussigne?identifiant='.$elt[2]) : ''; ?></td>
	       <td><?php echo ($elt[4]) ? link_to($elt[5], 'vrac/rechercheSoussigne?identifiant='.$elt[4]) : ''; ?></td>
	       <td><?php echo ($elt[6]) ? link_to($elt[7], 'vrac/rechercheSoussigne?identifiant='.$elt[6]) : ''; ?></td>
