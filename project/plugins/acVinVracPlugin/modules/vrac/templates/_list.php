    ...
    <tbody>
        ...
        foreach ($vracs->rows as $value) {
            $elt = $value->getRawValue()->value;
                ...
                ?>
                <tr data-words='<?php echo json_encode(array_merge(Search::getWords($elt[VracHistoryView::VALUE_ACHETEUR_NOM]), Search::getWords($elt[VracHistoryView::VALUE_VENDEUR_NOM]), Search::getWords($elt[VracHistoryView::VALUE_MANDATAIRE_NOM]), Search::getWords($elt[VracHistoryView::VALUE_PRODUITLIBELLE]), array(strtolower($elt[VracHistoryView::VALUE_ACHETEUR_NOM]), strtolower($elt[VracHistoryView::VALUE_VENDEUR_NOM]), strtolower($elt[VracHistoryView::VALUE_MANDATAIRE_NOM]), strtolower($elt[VracHistoryView::VALUE_PRODUITLIBELLE]), strtolower(($elt[VracClient::VRAC_VIEW_NUMARCHIVE]) ? $elt[VracClient::VRAC_VIEW_NUMARCHIVE] : ""))), JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) ?>' id="<?php echo vrac_get_id($value) ?>" class="<?php echo statusCssClass($elt[VracHistoryView::VALUE_STATUT]) ?> hamzastyle-item" >
