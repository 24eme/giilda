...
<?php if (!$valide): ?>
    ...
    <?php endif; ?>
    <td>
		<a href="<?php echo url_for('drm_init', array('identifiant' => $sf_user->getTiers()->getIdentifiant() , 'campagne_rectificative' => $campagne_rectificative)); ?>">Accéder à la déclaration en cours</a><br />
	   </td>
	   <td style="border: 0px; padding-left: 0px;background-color: #ffffff;">
	       <a href="<?php echo url_for('drm_delete', array('identifiant' => $sf_user->getTiers()->getIdentifiant(), 'campagne_rectificative' => $campagne_rectificative)); ?>" class="btn_annuler btn_majeur">Supprimer</a>
	   </td>
<?php else: ?>
	<td>OK</td>
    <?php if ($sf_user->hasCredential(myUser::CREDENTIAL_ADMIN)): ?>
