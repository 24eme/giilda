...
admindn=$(grep [^e]dn: config/app.yml | sed 's/.*dn: *//' | sed 's/"//g')
adminpass=$(grep -A 5  ldap config/app.yml | grep pass:  | sed 's/.*pass: *//' | sed 's/"//g' )
basedc=$(grep dc: config/app.yml | sed 's/.*dc: *//' | sed 's/"//g')
