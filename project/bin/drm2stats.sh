...
if ! test "$DRM_STAT_SOURCE_DIR" ; then
    echo DRM_STAT_SOURCE_DIR missing 1>&2
    exit 1
fi
if ! test "$DRM_STAT_DEST_STATS" ; then
...
echo "date;type;periode;operateur;accises;certification;genre;appellation;mention;lieu;couleur;cepage;Libelle complementaire;libelle complet;type DRM;categorie de mouvement;type de mouvement;volume en hl;pays export;numero du contrat;numero de document;prix de la transaction;accises acheteur;nom acheteur;fin 1;type DRM 2;categorie de mouvement 2;type de mouvement 2;TAV;pays export 2;fin 2;" > $DRM_STAT_DEST_STATS"/drm_stocks.csv"
cp $DRM_STAT_DEST_STATS"/drm_stocks.csv" $DRM_STAT_DEST_STATS"/drm_mouvements.csv"
join -a 1 -j 1 -t ';' $DRM_STAT_DEST_STATS"/.raw_stocks.csv" $DRM_STAT_DEST_STATS"/.raw_tav.csv"  | awk -F ';' 'BEGIN{OFS=";" ;} {$18 = "-"; sub(/,/, ".", $12); print $0}' | sed 's/£/;/g' >> $DRM_STAT_DEST_STATS"/drm_stocks.csv"
...
cat $DRM_STAT_DEST_STATS"/.raw_mouvements.csv" | sort >> $DRM_STAT_DEST_STATS"/drm_mouvements.csv"
