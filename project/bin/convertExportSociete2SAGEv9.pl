...
while(<STDIN>) {
	...
	print uc(encode_utf8(substr(decode_utf8($field[4]), 0, 35)))."\n";
	print "complément adresse (VINSI) : " if ($verbose);
	print uc(encode_utf8(substr(decode_utf8($field[5]), 0, 35)))."\n";
	print "code postal (VINSI) : " if ($verbose);
	print $field[6]."\n";
	print "ville (VINSI) : " if ($verbose);
	print uc(encode_utf8(substr(decode_utf8($field[7]), 0, 35)))."\n";
	print "région : " if ($verbose);
	print "\n";
	print "pays (VINSI) : " if ($verbose);
	print uc(substr(encode_utf8(decode_utf8($field[8])), 0, 35))."\n";
