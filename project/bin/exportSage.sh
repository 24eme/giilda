...
if test "$SAMBA_IP" && test "$SAMBA_SHARE" && test "$SAMBA_AUTH" && test "$SAMBA_SAGESUBDIR" && test "$SAMBA_SAGEFILE"; then
    ...
    if test "$SAMBA_SAGEVERIFY" && test "$(smbclient //$SAMBA_IP/$SAMBA_SHARE -A $SAMBA_AUTH -c "cd $SAMBA_SAGESUBDIR ; ls  $VINSIEXPORT" | grep -o "NT_STATUS_NO_SUCH_FILE")" != "NT_STATUS_NO_SUCH_FILE" ; then
