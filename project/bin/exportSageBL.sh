...
cat $TMP/societesWithSageData.csv | perl bin/convertExportSociete2SAGE.pl | iconv -f UTF8 -t ISO88591//TRANSLIT | sed 's/$/\r/' > $TMP/societes.sage

php symfony export:facture --horstaxe=true > $TMP/factures.csv
cat $TMP/factures.csv | perl bin/convertExportFacture2BLSAGE.pl | iconv -f UTF8 -t ISO88591//TRANSLIT | sed 's/$/\r/' > $TMP/factures.sage
