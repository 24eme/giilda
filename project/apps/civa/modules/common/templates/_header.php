<?php if(sfConfig::get('app_url_header')): ?>
    ...
    <?php if($sf_request->getAttribute('sf_route') && $sf_request->getAttribute('sf_route')->getRawValue() instanceof InterfaceEtablissementRoute && $sf_request->getAttribute('sf_route')->getEtablissement()):
        ...
        $etablissement = $sf_request->getAttribute('sf_route')->getEtablissement();
    endif; ?>
    ...
    <div id="main">
        <div style="position:relative;" id="nav">
            <?php if($compte): ?>
                ...
                <?php if(sfConfig::get('sf_debug')): ?>
                    <a style="position: absolute; right: 10px; top: -15px; font-size: 10px; color: #ff0000;" href="<?php echo sfConfig::get('app_url_nav')."?compte=".$compte."&isAdmin=".($sf_user->hasCredential(AppUser::CREDENTIAL_ADMIN))."&active=".$sf_request->getParameter('module') ?>&etablissement=<?php echo $etablissement->_id ?>">[voir l'url de la nav]</a></pre>
                <?php endif; ?>
