        ...
        <div style="position:relative;" id="nav">
            <?php if($compte): ?>
                ...
                <?php if(sfConfig::get('sf_debug')): ?>
                    <a style="position: absolute; right: 10px; top: -15px; font-size: 10px; color: #ff0000;" href="<?php echo sfConfig::get('app_url_nav')."?compte=".$compte."&active=".$sf_request->getParameter('module') ?>">[voir l'url de la nav]</a></pre>
                <?php endif; ?>
                <?php if (preg_match('/drm/', $sf_request->getParameter('module'))) : ?>
